...
def test_campaign_lifecycle(test_client, auth_user):
    ...
    response = test_client.post(
        "/campaigns/new", data={"name": "Integration Test Campaign", "type": "test"}
    )
    assert response.status_code in (
        200,
        201,
        302,
    ), f"Expected 200/201/302, got {response.status_code}"
    ...
    with test_client.application.app_context():
        ...
        campaign = Campaign.query.filter_by(
            name="Integration Test Campaign", created_by=user.id
        ).first()
...
def test_notification_toast_flow(test_client, auth_user):
    ...
    response = test_client.post(
        "/api/notifications/toast/success", json={"message": "Integration test success"}
    )
    assert response.status_code == 201
