...
PREPARE_CHECKLIST = [
    {
        "id": "contacts_attached",
        "label": "Contact list attached",
        "check": lambda c: c.contact_list_id is not None,
    },
    {
        "id": "contacts_loaded",
        "label": "Contacts loaded (>0)",
        "check": lambda c: (c.readiness or {}).get("contacts_loaded", 0) > 0,
    },
    {
        "id": "contacts_duplicates",
        "label": "Duplicates <= 5% of loaded",
        "check": lambda c: (c.readiness or {}).get("duplicate_rate", 0) <= 0.05,
    },
    {
        "id": "contacts_invalid",
        "label": "Invalid contacts = 0",
        "check": lambda c: (c.readiness or {}).get("invalid_count", 0) == 0,
    },
    {
        "id": "contacts_dnc",
        "label": "DNC/blocklist ran",
        "check": lambda c: (c.readiness or {}).get("dnc_ran", False),
    },
    {
        "id": "audio_intro",
        "label": "Intro audio attached",
        "check": lambda c: bool((c.settings or {}).get("audio_intro_id")),
    },
    {
        "id": "audio_hold",
        "label": "Hold music attached",
        "check": lambda c: bool((c.settings or {}).get("audio_hold_id")),
    },
    {
        "id": "audio_agent",
        "label": "Agent connect audio attached",
        "check": lambda c: bool((c.settings or {}).get("audio_agent_id")),
    },
    {
        "id": "audio_voicemail",
        "label": "Voicemail audio attached",
        "check": lambda c: bool((c.settings or {}).get("audio_voicemail_id")),
    },
    {
        "id": "audio_outro",
        "label": "Outro audio attached",
        "check": lambda c: bool((c.settings or {}).get("audio_outro_id")),
    },
    {
        "id": "sms_template",
        "label": "SMS template set",
        "check": lambda c: bool((c.settings or {}).get("sms_template")),
    },
    {
        "id": "sms_variables",
        "label": "SMS variables valid",
        "check": lambda c: True,
    ...
    {"id": "sms_preview", "label": "SMS preview renders", "check": lambda c: True},
    {
        "id": "sms_length",
        "label": "SMS <= 160 chars",
        "check": lambda c: len((c.settings or {}).get("sms_template", "")) <= 160,
    },
    {
        "id": "email_subject",
        "label": "Email subject set",
        "check": lambda c: bool((c.settings or {}).get("email_subject")),
    },
    {
        "id": "email_body",
        "label": "Email body set",
        "check": lambda c: bool((c.settings or {}).get("email_body")),
    },
    {"id": "email_images", "label": "Email images attached", "check": lambda c: True},
    {
        "id": "email_attachments",
        "label": "Email attachments OK",
        "check": lambda c: True,
    },
    {
        "id": "caller_profile",
        "label": "Caller profile selected",
        "check": lambda c: c.caller_profile_id is not None,
    },
    {
        "id": "number_pool",
        "label": "Number pool non-empty",
        "check": lambda c: (c.readiness or {}).get("pool_size", 0) > 0,
    },
    {
        "id": "provider_voice",
        "label": "Voice provider connected",
        "check": lambda c: (c.readiness or {}).get("voice_provider_connected", False),
    },
    {
    ...
    {
        "id": "rules_concurrency",
        "label": "Concurrency > 0",
        "check": lambda c: (c.settings or {}).get("concurrent_calls", 0) > 0,
    },
    {
        "id": "rules_retries",
        "label": "Retries configured",
        "check": lambda c: (c.settings or {}).get("retry_attempts") is not None,
    },
    {
        "id": "rules_business_hours",
        "label": "Business hours resolved",
        "check": lambda c: True,
    },
]
...
VERIFY_CHECKS = [
    {
        "id": "verify_audio",
        "label": "Audio files exist and are decodable",
        "check": lambda c: True,
    },
    {
        "id": "verify_numbers",
        "label": "Numbers are E.164, no dupes/invalid",
        "check": lambda c: True,
    },
    {
        "id": "verify_sip",
        "label": "SIP/provider voice health OK",
        "check": lambda c: True,
    },
    {
        "id": "verify_internet",
        "label": "Internet connectivity (best-effort)",
        "check": lambda c: True,
    },
    {
        "id": "verify_provider",
        "label": "Messaging provider health when used",
        "check": lambda c: True,
    },
    {"id": "verify_api_keys", "label": "API keys non-empty", "check": lambda c: True},
    {
        "id": "verify_ffmpeg",
        "label": "FFmpeg available on host",
        "check": lambda c: os.path.exists("/usr/bin/ffmpeg"),
    },
    {
        "id": "verify_permissions",
        "label": "Upload dir writable",
        "check": lambda c: os.access(
            os.path.join(
                os.path.dirname(
                    os.path.dirname(
...
class CampaignService:
    ...
    def prepare_campaign(campaign_id):
        ...
        for check in PREPARE_CHECKLIST:
            ...
            results.append(
                {"id": check["id"], "label": check["label"], "passed": passed}
            )
            if not passed:
                all_pass = False
        ...
        campaign.readiness = {
            ...
            "voice_provider_connected": (campaign.readiness or {}).get(
                "voice_provider_connected", False
            ),
        }
    ...
    def verify_campaign(campaign_id):
        ...
        return {
            ...
            "all_passed": all_pass,
            "verified_at": (
                campaign.verified_at.isoformat() if campaign.verified_at else None
            ),
        }
    ...
    def launch_campaign(campaign_id):
        ...
        if not campaign.verified_at:
            return {
                "error": "Campaign must be verified before launch",
                "code": "not_verified",
            }
